...
function naturalMerge(a, descPos, descCount) {
  ...
  while (runs.length - 1 > 1) {
    ...
    for (let i = 0; i < runs.length - 1; i += 2) {
      const l = runs[i];
      if (i + 1 >= runs.length - 1) { nr.push(l); continue; }
      const m = runs[i + 1];
...
function countingSort(a, minv) {
  const n = a.length;
  let mx = a[0], mn = a[0];
  for (let i = 1; i < n; i++) { mx = Math.max(mx, a[i]); mn = Math.min(mn, a[i]); }
  const range = mx - mn + 1;
  const cnt = new Array(range).fill(0);
  for (let i = 0; i < n; i++) cnt[a[i] - mn]++;
  let sum = 0;
  for (let i = 0; i < range; i++) { const c = cnt[i]; cnt[i] = sum; sum += c; }
...
function radixSort(a) {
  ...
  let mn = a[0], mx = a[0];
  for (let i = 1; i < n; i++) { mn = Math.min(mn, a[i]); mx = Math.max(mx, a[i]); }
  const offset = mn < 0 ? -mn : 0;
  const src = new Array(n), dst = new Array(n);
  let maxKey = 0;
  for (let i = 0; i < n; i++) { const k = a[i] + offset; src[i] = k; if (k > maxKey) maxKey = k; }
  let bytes = 4;
  ...
  for (let b = 0; b < bytes; b++) {
    ...
    let s = 0;
    for (let i = 0; i < 256; i++) { const c = cnt[i]; cnt[i] = s; s += c; }
    for (let i = 0; i < n; i++) { const v = src[i]; dst[cnt[(v >>> shift) & 0xff]++] = v; }
...
function heapify(a, start, end, i) {
  for (;;) {
    let largest = i;
    const l = 2 * i - start + 1, r = 2 * i - start + 2;
...
function heapSortRange(a, l, r) {
  ...
  for (let i = r; i > l; i--) { [a[l], a[i]] = [a[i], a[l]]; heapify(a, l, i, l); }
}

// ---------- Introsort（快排 + 堆回退 + 插入基） ----------
function introRec(a, l, r, depth) {
  if (r - l <= 16) { insertionSort(a, l, r); return; }
...
function lxySortImpl(a, stable) {
  ...
  let maxv = a[0], minv = a[0];
  let descCount = 0;
  const descPos = [];
  const descLimit = (n / 16) > 16 ? (n / 16) : 16;
  for (let i = 1; i < n; i++) {
    if (a[i] < a[i - 1]) { if (descCount < 32) descPos[descCount] = i; descCount++; }
  ...
  if (dataRange <= n) { used = "Counting"; countingSort(a, minv); return; }

  // NaturalMerge（近有序）
  if (descCount <= descLimit && descCount <= 32) { used = "NaturalMerge"; naturalMerge(a, descPos, descCount); return; }

  // Radix（宽范围，大数组）
  if (n >= 512) { used = "Radix"; radixSort(a); return; }
